<div class="relative" x-data="{ open: @entangle('isOpen') }" @click.outside="open = false">
    <!-- Bell Button -->
    <button @click="open = !open; $wire.toggleDropdown()" class="relative p-2 text-slate-400 hover:text-white transition-colors rounded-lg hover:bg-slate-800">
    ...
    <div x-show="open" 
        ...
        <div class="px-4 py-3 border-b border-slate-700/50 flex justify-between items-center bg-slate-800/80">
            ...
            @if($unreadCount > 0)
                <span class="bg-red-500/20 text-red-400 py-0.5 px-2 rounded-full text-xs font-medium">{{ $unreadCount }} Baru</span>
            @endif
        </div>
        ...
        <div class="max-h-80 overflow-y-auto divide-y divide-slate-700/50">
            @forelse($notifications as $notification)
                ...
                @endphp
                <button wire:click="markAsRead('{{ $notification->id }}', '{{ $data['url'] ?? '#' }}')" class="w-full text-left px-4 py-3 hover:bg-slate-700/50 transition-colors flex gap-3 group relative">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-medium text-white group-hover:text-indigo-300 transition-colors">{{ $data['title'] ?? 'Notifikasi Baru' }}</p>
        ...
        @if($unreadCount > 0)
            <div class="p-2 border-t border-slate-700/50 bg-slate-800/80">
                <button wire:click="markAllAsRead" class="w-full text-center px-4 py-2 text-xs font-medium text-slate-400 hover:text-white hover:bg-slate-700/50 rounded-lg transition-colors">
                    Tandai semua dibaca
                </button>
            </div>
        @endif
