    ...
    <div class="glass-card p-6 mb-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex-1 w-full relative">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" class="input-field pl-10 py-2 w-full max-w-md" placeholder="Cari berdasarkan nama, NIK, atau email...">
            </div>
                        ...
                        <tr class="hover:bg-slate-800/30 transition-colors">
                            <td class="py-3 px-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center border border-slate-600">
                                        <span class="text-lg font-bold text-slate-300">{{ substr($user->name, 0, 1) }}</span>
                                    </div>
                                    <div>
                            ...
                            <td class="py-3 px-4">
                                @foreach($user->roles as $role)
                                    ...
                                    @endphp
                                    <span class="inline-block px-2.5 py-1 rounded-full text-xs font-medium border {{ $color }}">
                                        {{ ucfirst(str_replace('_', ' ', $role->name)) }}
                                    </span>
                                @endforeach
                            </td>
                            <td class="py-3 px-4 text-right">
                                <button wire:click="openRoleModal({{ $user->id }})" class="btn-primary py-1.5 px-3 text-xs flex items-center gap-1 inline-flex">
                ...
                @if($selectedUser)
                    <div class="mb-4 p-3 bg-slate-700/30 rounded-lg flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center">
                            <span class="text-lg font-bold text-slate-300">{{ substr($selectedUser->name, 0, 1) }}</span>
                        </div>
                        <div>
                    ...
                    <div class="mb-4">
                        ...
                        @error('newRole') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                @endif
