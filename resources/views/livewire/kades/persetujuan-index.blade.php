                    ...
                    @forelse($pengajuan as $item)
                        <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors">
                            ...
                            <td class="py-4 px-6 text-right whitespace-nowrap">
                                ...
                                <button wire:click="setuju({{ $item->id }})" wire:confirm="Anda yakin menyetujui surat ini?" 
                                    wire:loading.attr="disabled" wire:target="setuju({{ $item->id }})"
                                    class="inline-flex items-center px-4 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-xs font-medium transition-colors disabled:opacity-70 disabled:cursor-not-allowed">
                                    <span wire:loading.remove wire:target="setuju({{ $item->id }})">ACC Surat</span>
                                    <span wire:loading.inline-flex wire:target="setuju({{ $item->id }})" class="items-center">
